...
# ===========================================================================
# Landslide Alert Engine — TerraWatch SMS Gateway
# ===========================================================================

class LandslideAlerter:
    """
    Stateful SMS sender for landslide telemetry events.

    Triggers an SMS when:
      (a) event status is 'LANDSLIDE ALERT', AND
      (b) EITHER a state transition from non-alert → alert occurred,
          OR the debounce window (SMS_DEBOUNCE_SECONDS) has elapsed.

    Shares Twilio config, debounce, and SMS_SEND_ONCE from config.py.
    """

    def __init__(self):
        self._last_sms_time: float = 0.0
        self._last_status: str = "NORMAL"
        self._alert_sent_count: int = 0

    def handle_event(self, event: dict):
        """Evaluate a landslide_reading event and send SMS if warranted."""
        if event.get("type") != "landslide_reading":
            return

        status = str(event.get("status", "NORMAL")).upper()

        # Gate 0 — auto-dispatch disabled
        if not config.AUTO_DISPATCH_ENABLED:
            self._last_status = status
            return

        # Gate 1 — only care about LANDSLIDE ALERT
        if "LANDSLIDE" not in status:
            self._last_status = status
            return

        # Gate 2 — single-SMS quota protection
        if config.SMS_SEND_ONCE and self._alert_sent_count >= 1:
            log.info("Landslide SMS skipped — SMS_SEND_ONCE quota reached")
            self._last_status = status
            return

        now = time.time()
        time_since_last = now - self._last_sms_time
        transitioned = "LANDSLIDE" not in self._last_status

        should_send = transitioned or (time_since_last >= config.SMS_DEBOUNCE_SECONDS)

        if should_send:
            res = self._send_landslide_sms(event)
            self._last_sms_time = now
            if res and res.get("success"):
                self._alert_sent_count += 1
                log.info(
                    f"Landslide alert SMS delivered! "
                    f"(Total sent this session: {self._alert_sent_count})"
                )
        else:
            remaining = config.SMS_DEBOUNCE_SECONDS - time_since_last
            log.debug(f"Landslide SMS debounced — {remaining:.0f}s until next allowed SMS")

        self._last_status = status

    def _format_landslide_message(self, event: dict) -> str:
        """Build a TerraWatch-branded SMS body."""
        ts_raw = event.get("timestamp", datetime.now(timezone.utc).isoformat())
        try:
            ts_obj = datetime.fromisoformat(ts_raw)
            from datetime import timedelta
        ...
        node_id       = event.get("node_id", "NODE_B")
        soil          = event.get("soil_moisture", "--")
        tilt          = event.get("tilt_angle", "--")
        vib           = event.get("vibration", "--")
        smoothed_prob = event.get("smoothed_probability", 0)
        risk_pct      = f"{smoothed_prob * 100:.1f}" if isinstance(smoothed_prob, (int, float)) else "--"
        trend         = event.get("trend", "--")

        return (
            f"🌍 *TERRAWATCH LANDSLIDE ALERT*\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📍 *Sensor Node:* `{node_id}`\n"
            f"💧 *Soil Moisture:* {soil} ADC\n"
            f"📐 *Tilt Angle:* {tilt}°\n"
            f"📳 *Vibration:* {vib} g\n"
            f"🎯 *AI Risk:* {risk_pct}%  |  Trend: {trend}\n"
            f"🕒 *Timestamp:* {ts_str}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🚨 *Ground movement detected — immediate inspection required!*"
        )

    def _send_landslide_sms(self, event: dict, target_number: str = None) -> dict:
        """Dispatch Twilio SMS for a landslide alert. Mirrors SMSAlerter._send_sms()."""
        import database
        ...
        if not recipients:
            log.warning("No landslide alert recipients registered.")
            return {
                "success": False,
                "twilio_configured": False,
                "message": "No mobile numbers registered. Please register a number on the dashboard.",
            }
        ...
        if not twilio_ready:
            log.warning("Twilio not configured — landslide alert not sent.")
            return {
                "success": False,
                "twilio_configured": False,
                "recipients": recipients,
                "message": f"Recipient(s) registered. Add TWILIO credentials in .env for SMS delivery.",
            }

        message_body = self._format_landslide_message(event)
        sent_sids = []
        errors = []
        ...
        try:
            ...
            for phone in recipients:
                ...
                if config.ALERT_CHANNEL in ("whatsapp", "both"):
                    ...
                    try:
                        ...
                        sent_sids.append(msg.sid)
                        log.info(f"Landslide WhatsApp alert sent to {wa_to}! SID={msg.sid}")
                    except Exception as wa_err:
                        log.error(f"Failed to send landslide WhatsApp to {wa_to}: {wa_err}")
                        errors.append(f"WhatsApp {wa_to}: {wa_err}")
                ...
                if config.ALERT_CHANNEL in ("sms", "both"):
                    try:
                        ...
                        sent_sids.append(msg.sid)
                        log.info(f"Landslide SMS sent to {clean_phone}! SID={msg.sid}")
                    except Exception as sms_err:
                        log.error(f"Failed to send landslide SMS to {clean_phone}: {sms_err}")
                        errors.append(f"SMS {clean_phone}: {sms_err}")

            success    = len(sent_sids) > 0
            channel    = "WhatsApp" if config.ALERT_CHANNEL == "whatsapp" else ("WhatsApp & SMS" if config.ALERT_CHANNEL == "both" else "SMS")
            msg_text   = f"Landslide {channel} alert sent to {len(sent_sids)} destination(s): {', '.join(recipients)}"
            if errors and not success:
                msg_text = f"Failed to send: {'; '.join(errors)}"
            ...
            log.error("twilio package not installed. Run: pip install twilio")
            return {"success": False, "twilio_configured": True, "message": "twilio not installed"}
        except Exception as exc:
            log.exception(f"Failed to send landslide SMS via Twilio: {exc}")
            return {"success": False, "twilio_configured": True, "message": str(exc)}


# ---------------------------------------------------------------------------
# Module-level landslide alerter instance (used by app.py)
# ---------------------------------------------------------------------------
_landslide_alerter = LandslideAlerter()


def send_landslide_sms(event: dict, target_number: str = None) -> dict:
    """
    Module-level function for landslide SMS dispatch.
    app.py calls: from sms_alert import send_landslide_sms
    """
    if target_number:
        return _landslide_alerter._send_landslide_sms(event, target_number=target_number)
    _landslide_alerter.handle_event(event)
    return {"status": "handled"}
